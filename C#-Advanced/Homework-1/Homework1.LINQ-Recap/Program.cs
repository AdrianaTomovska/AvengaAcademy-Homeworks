using Homework1.LINQ_Recap.Enums;
using Homework1.LINQ_Recap.Models;
...
var firstProductAbove100 = products.FirstOrDefault(c => c.Category == ProductCategory.Fragrances && c.Price > 100);
Console.WriteLine($"Title of the first product in the Fragrances category with a price above 100: {firstProductAbove100.Title}");
Console.ReadLine();


//2.Select the brand of the last product that has a stock lower than 40.

var lastProductStockLower40 = products.LastOrDefault(p => p.Stock < 40);
Console.WriteLine($"Brand of the last product that has a stock lower than 40: {lastProductStockLower40.Brand}");
Console.ReadLine();
...
var descriptionOfTheFirstProduct = products.FirstOrDefault(p => p.Rating == 4.43);
Console.WriteLine($"The description of the first product with a rating equal to 4.43: {descriptionOfTheFirstProduct.Description}");
Console.ReadLine();
...
var titleOfLastSkincarePorduct = products.LastOrDefault(p =>p.Category == ProductCategory.Skincare && p.Price < 50);
Console.WriteLine($"Title of the last Skincare product with a price under $50: {titleOfLastSkincarePorduct.Title}");
Console.ReadLine();
...
var raitingOfFirstProductBrand = products.FirstOrDefault(p => p.Brand.Contains("Apple"));
Console.WriteLine($"Rating of the first product with a brand that contains the word Apple {raitingOfFirstProductBrand.Rating}");
Console.ReadLine();
...
var descriptionOfLastProduct = products.LastOrDefault(p => p.Description.ToLower().Contains("display"));
Console.WriteLine($"Description of the last product with \"display\" in the description {descriptionOfLastProduct.Description}");
Console.ReadLine();


//7.Select the title of the first Laptop that has a stock greater than 80.

var titleOfTheFirstLaptop = products.FirstOrDefault(p => p.Category == ProductCategory.Laptops && p.Stock > 80);
Console.WriteLine($"Title of the first Laptop that has a stock greater than 80: {titleOfTheFirstLaptop.Title}");
Console.ReadLine();


//8.Get the brand of the last product with "Pro" in the title.

var lastProductWithPro = products.LastOrDefault(p => p.Title.ToLower().Contains("pro"));
Console.WriteLine($"Brand of the last product with \"Pro\" in the title {lastProductWithPro.Brand}");
Console.ReadLine();


//9.Retrieve the title of the first product that has a price above $1200.

var firstProductPriceAbove1200 = products.FirstOrDefault(p => p.Price > 1200);
Console.WriteLine($"Title of the first product that has a price above $1200: {firstProductPriceAbove1200.Title}");
Console.ReadLine();
...
var stockCount = products.LastOrDefault(p => p.Category == ProductCategory.Smartphones);
Console.WriteLine($"Stock count of the last product that belongs to the Smartphones category {stockCount.Stock}");
Console.ReadLine();
...
var brandNameStartWithH = products.FirstOrDefault(p => p.Brand.StartsWith("H"));
Console.WriteLine($"Description of the first product with a brand name starting with 'H' {brandNameStartWithH.Description}");
Console.ReadLine();


//12.Retrieve the price of the last product that has "Essence" in its title.

var priceOfLastProductEssence = products.LastOrDefault(p =>p.Title.ToLower().Contains("essence"));
Console.WriteLine($"Price of the last product that has \"Essence\" in its title {priceOfLastProductEssence.Price}");
Console.ReadLine();


//13.Select the brand of the first product with a description longer than 100 characters.

var descriptionLongerThan100 = products.FirstOrDefault(p => p.Description.Length > 100);
Console.WriteLine($"Brand of the first product with a description longer than 100 characters: {descriptionLongerThan100.Brand}");
Console.ReadLine();


//14.Get the title of the last product with a rating below 4.1 and stock over 30.

var lastProductRaitingAndStock = products.LastOrDefault(p => p.Rating < 4.1 && p.Stock > 30);
Console.WriteLine($"Title of the last product with a rating below 4.1 and stock over 30: {lastProductRaitingAndStock.Title}");
Console.ReadLine();


//15.Retrieve the description of the first product that has "Serum" in the title.

var pruductSerum = products.FirstOrDefault(p => p.Title.ToLower().Contains("serum"));
Console.WriteLine($"Description of the first product that has \"Serum\" in the title: {pruductSerum.Description}");
Console.ReadLine();
...
var productDetailsList = products.Select(p => new ProductDetails(p.Id, p.Title, p.Price)).ToList();
